...
import {
  Building2, Users, ShoppingBag, DollarSign, Bell, Moon,
  CheckCircle2, AlertTriangle, TrendingUp, MapPin, Clock,
} from "lucide-react";
import { getQueue, getByStatus, type PendingEntry } from "@/lib/adminData";

const monthlyGMV = [
  { m: "Jan", v: 62 }, { m: "Feb", v: 70 }, { m: "Mar", v: 52 },
  { m: "Apr", v: 78 }, { m: "May", v: 65 }, { m: "Jun", v: 84 },
  { m: "Jul", v: 72 }, { m: "Aug", v: 88 }, { m: "Sep", v: 75 },
  { m: "Oct", v: 92 }, { m: "Nov", v: 80 }, { m: "Dec", v: 95 },
];
...
export default function AgriConnectDashboard() {
  ...
  function load() {
    ...
    setPendingApprovals(getByStatus("Pending"));
  }
  ...
  const cooperativeCount = allEntries.filter((e) => e.type === "Cooperative").length;
  const buyerCount = allEntries.filter((e) => e.type === "Buyer").length;
  const approvedCount = getByStatus("Approved").length;

  // Real counts from cooperative localStorage data
  const coopRaw = typeof window !== "undefined"
    ? (() => { try { return JSON.parse(localStorage.getItem("agriconnect.cooperativeData") ?? "{}"); } catch { return {}; } })()
    : {};
  const memberCount = Array.isArray(coopRaw.members) ? coopRaw.members.length : 0;

  const stats = [
    { label: "Registered Cooperatives", value: String(cooperativeCount || "—"), delta: `${approvedCount} approved`, icon: Building2, tone: "green" },
    { label: "Farmer Members", value: String(memberCount || "—"), delta: "from cooperative data", icon: Users, tone: "green" },
    { label: "Active Buyers", value: String(buyerCount || "—"), delta: "registered buyers", icon: ShoppingBag, tone: "green" },
        ...
        <div className="flex-1 min-w-0 p-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-5">
            ...
            <Card>
              ...
              {[
                { label: "Platform uptime", value: "99.98%", ok: true },
                { label: "SMS delivery rate", value: "97.4%", ok: true },
              ...
              {pendingApprovals.length === 0 ? (
                ...
                pendingApprovals.slice(0, 4).map((a) => (
                  <div key={a.id} className="flex items-center justify-between py-3 border-b border-zinc-800 last:border-b-0 gap-3">
                    ...
                    <span className={`text-[11.5px] font-semibold px-2.5 py-1 rounded-md whitespace-nowrap ${stageStyle}`}>
                      Pending
                    </span>
                  </div>
                ))
