...
export default function ForgotPasswordPage() {
  ...
  const handleSendOtp = () => {
    ...
    setOtpSent(true);
    setMessage(`${copy.codeSent} ${email}.`);
  };
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!result.ok) {
      setMessage(copy.accountNotFound);
      return;
    }
  ...
  return (
    ...
    >
      ...
      <section className="relative w-full max-w-md rounded-2xl border border-gray-200 dark:border-emerald-400/30 bg-white dark:bg-[#071b0f]/95 p-6 shadow-2xl shadow-black/10 dark:shadow-black/40 sm:p-8">
        <div className="mb-6">
          <Link href="/" className="text-xs font-semibold text-emerald-700 hover:underline dark:text-emerald-300">{copy.home}</Link>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-7 space-y-4">
          ...
          <div>
            <div className="mb-1.5 flex items-center justify-between gap-3">
              <label className={labelClass}>{copy.verificationCode}</label>
              {otpSent && <button type="button" onClick={handleSendOtp} className="text-xs font-medium text-emerald-600 dark:text-emerald-400 hover:underline">{copy.resendCode}</button>}
            </div>
            <input value={verificationCode} onChange={(event) => setVerificationCode(event.target.value.replace(/\D/g, "").slice(0, 6))} inputMode="numeric" autoComplete="one-time-code" placeholder={copy.codePlaceholder} required className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>{copy.newPassword}</label>
            <input type="password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} placeholder={copy.newPasswordPlaceholder} minLength={8} required className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>{copy.confirmPassword}</label>
            <input type="password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} placeholder={copy.confirmPasswordPlaceholder} minLength={8} required className={inputClass} />
          </div>
        ...
        <p className="mt-6 text-center text-sm text-gray-500 dark:text-emerald-100/60">
          {copy.remembered} <Link href="/login" className="font-medium text-emerald-600 dark:text-emerald-400 hover:underline">{copy.backToLogin}</Link>
        </p>
      </section>
    </main>
  );
